...
const SignUp = () => {
  ...
  const boxStyle1 = { position: "relative", height: "100vh", overflow: "hidden", display: "flex", justifyContent: "center", alignItems: "center" };
  ...
  return (
    <Box sx={boxStyle1}>
      ...
      <Box sx={boxStyle2}>
        <Typography variant="h4" sx={{ color: "#fff", marginBottom: "20px" }}> Sign Up </Typography>
        <form onSubmit={handleSignup}>
        <TextField onChange={(e)=>{setUsername(e.target.value)}} name='name' required label="Username" type='text' fullWidth style={inputStyle}/>
        <TextField onChange={(e)=>{setEmail(e.target.value)}} name='email' required label="Email" type="email"  fullWidth style={inputStyle} />
